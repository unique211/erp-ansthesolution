$(document).ready(function() {
    //alert("HII");
    var table_name="salesreturn_master";
    datashow();
    ...
    $(document).on("click","#add",function(e){        
        ...
        if(batchvalue > 0){
            ...
            var markup = '<tr>'+
            '<td id="groupid_'+sr+'" style="display:none;" >'+groupid+'</td>'+//1
            '<td id="itemid_'+sr+'" style="display:none;" >'+item_id+'</td>'+//2
            '<td width="5%"></td>'+//3
            '<td width="10%" id="groupname'+sr+'">'+groupname+'</td>'+//4
            '<td width="15%" id="itemname'+sr+'">'+item_name+'</td>'+//5
            '<td width="10%" id="mrp'+sr+'">'+mrp+'</td>'+//6
            '<td width="5%" id="qty_'+sr+'">'+qty+'</td>'+//7
            '<td id="discount_'+sr+'" style="display:none;" >'+discount+'</td>'+//8
            '<td id="cgst1_'+sr+'" style="display:none;" >'+cgst+'</td>'+//9
            '<td id="sgst1_'+sr+'" style="display:none;" >'+sgst+'</td>'+//10
            '<td width="10%" id="discountamt_'+sr+'" >'+disamt+'</td>'+//11
            '<td width="7%" id="cgst_'+sr+'" >'+cgst1+'</td>'+//12
            '<td width="8%" id="sgst_'+sr+'" >'+sgst2+'</td>'+//13
            '<td width="10%" id="total_'+sr+'">'+total+'</td>'+//14
            '<td hidden id="'+sr+'">'+sr+'</td>'+//15
            '<td width="20%" ><button type="button" name="edit" class="edit_data_pur btn btn-xs btn-success" id="'+sr+'"><i class="fa fa-edit"></i></button>  <button name="delete" value="Delete" class="dele_data1 btn btn-xs btn-danger" ><i class="fa fa-trash"></i></button></td>'+//16
            '</tr>';
            sr=sr+1;
                ...
                {                             
                    ...
                    $.ajax({
                        ...
                        data : {
                            ...
                            total:total, 
                            returnid:returnid,
                            table_name:'salesreturndetails',
                        },
                        success: function(data){          
                            successTost("Operation Successfull");                                
                        },
                        error:function(){
                            errorTost("Operation Failed for salesreturndetails" );
                        }
                    });													
                }
                successTost("Operation Successfull");
    ...
    var where="";
        $.ajax({
            type:"POST",
            url:base_url+"Controller/showallsalesreturn",
            data:{		
  ...
  // alert('where '+where);
    $.ajax({
        ...
        success: function(data){
            ...
            var totalamt=0;
            var cgstamt=0,cgstamt1=0;
            var sgstamt=0,sgstamt1=0;disless1=0;finalDisamt=0;
            var dis_less=$('#discountless').val();
            for(i=0;i<data.length; i++){
                sr=sr+1;
                
                totalamt=parseFloat(data[i].mrp)* parseFloat(data[i].qty);
                disamt=parseFloat(totalamt)*parseFloat(data[i].dis)/100;
                ...
                '<td id="discount_'+sr+'" style="display:none;" >'+data[i].dis+'</td>'+
                ...
                '<td width=10%" id="total_'+sr+'">'+data[i].total+'</td>'+
